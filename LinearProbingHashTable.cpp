...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<std::string> hashFn) {
    ...
    logicalSize = 0;
    elems = new Slot[hashingFunction.numSlots()]; // allocates memory to the hashing table.

    for (int i = 0; i < hashingFunction.numSlots(); i++) {
        elems[i].type =  SlotType::EMPTY; // Initializes all types to EMPTY
    }
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    int wrappedIndex = hashingFunction(elem);
    if (logicalSize == hashingFunction.numSlots()) return false;
    else if (contains(elem)) return false;
    else {
        for (int i = 0; i < hashingFunction.numSlots(); i++) { // Goes around the table fully, worst case, O(n).
            wrappedIndex = wrappedIndex % hashingFunction.numSlots(); // Finds the new index to look at if you've overshot the table, using modulo arithmetic

            //if not empty or tombstone, it has to be filled. otherwise, you have an error will assigning types to members of elems[].
            if (elems[wrappedIndex].type == SlotType::EMPTY || elems[wrappedIndex].type == SlotType::TOMBSTONE) {
                elems[wrappedIndex].value = elem;
                elems[wrappedIndex].type = SlotType::FILLED;
                logicalSize++;
                return true;
            }
            wrappedIndex++;
    ...
    return false; // no empty spaces in the table it's not in :(
}

/* contains()
 * looks through the hash table starting at the index that the given string should hash to. If
 * the string isn't there, it moves right until it either finds the string or an empty space
 * indicates that the string hasn't been shuffled to the right; it's not in the table at all.
 * @param elem, STRING that the function is called to search for
 * @return bool, tells whether or not the hash table contains the string elem.
 */
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    for (int i = 0; i < hashingFunction.numSlots(); i++) {
        ...
        wrappedIndex = wrappedIndex % hashingFunction.numSlots();

        if (elems[wrappedIndex].type == SlotType::EMPTY) return false;
        else if (elems[wrappedIndex].type == SlotType::FILLED && elems[wrappedIndex].value == elem) return true;
        wrappedIndex++;
    }
    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    for (int i = 0; i < hashingFunction.numSlots(); i++) {
        ...
        if (elems[wrappedIndex].value == elem && elems[wrappedIndex].type == SlotType::FILLED) {

            // Kills it without modifying the string.
            elems[wrappedIndex].type = SlotType::TOMBSTONE;
            logicalSize--;
            return true;
        }
        wrappedIndex++;
